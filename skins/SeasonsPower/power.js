/* javascript for the weewx SeasonsPower skin
 * Copyright (c) Tom Keffer, Matthew Wall
 * Distributed under terms of GPLv3.  See LICENSE.txt for your rights.
 */

const cookie_prefix = "weewx.power.";
...
function openPOWERFile(date) {
    if (date.match(/^\d\d\d\d/)) {
        window.location = "POWER/POWER-" + date + ".txt";
    }
}

function openTabularFile(date) {
    if (date.match(/^\d\d\d\d/)) {
        window.location = "tabular.html?report=POWER/POWER-" + date + ".txt";
    }
}
